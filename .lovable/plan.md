...
## Step 6 — Confirm clean: DONE

`git status -sb` shows `## main...origin/main` with no `[ahead N]`. Everything is pushed.

## Step 6b — The leftover `mirror/` folder is a duplicate

You are right. The live repo root already contains:
...
Those are exactly the three SimpleAssets folders plus the manifest sitting inside your local `mirror/`. It is the old nested copy from before we flattened the layout — the mirror expects files at the root, not under `mirror/`.

So: **do not push `mirror/`.** It would double the repo size for zero benefit.

Delete the stale copy so git stops reporting it:
...
You should then see `## main...origin/main` and nothing else. Git work is finished.

## Step 7 — I finish the app side

Already confirmed live: the root `manifest.json` on GitHub Pages returns 200 and carries the full **2575** entries. Remaining work on my side, no action needed from you:

1. Replace the stale 832-entry file list in `public/gpk-manifest.json` with the live 2575-entry list.
2. Re-run the mirror audit across primary (GitHub Pages), Netlify and Cloudflare and report which images each provider is missing.
3. Run a live import test to confirm resolution end to end.
